...
- Prelight is a distinct temporary `presence.prelight` input, not occupancy. Hosts configure a small set of targets/values and a bounded maximum duration. Lugn restores known prior values if prelight ends before occupancy, while `occupied`, `confirmed_empty`, explicit scenes, and manual light changes take control of the overlay.
- `occupied` triggers the minimum full-scene path and confirmed empty triggers physical off. Ordinary occupancy is not used to infer prelight.
